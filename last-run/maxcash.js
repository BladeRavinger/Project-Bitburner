...
export async function main(ns) {
 const location = "/last-run/";
	let servers = dpList(ns);
	// let target = "foodnstuff"
	ns.disableLog("ALL");
	for (let server of servers) {
		await ns.scp([`${location}bin.wk.js`, `${location}bin.hk.js`, `${location}bin.gr.js`], server,"home")
	}
				...
				if (ns.hasRootAccess(server) && ns.hasRootAccess(target) && ns.getServerMoneyAvailable(target) > 5000) {
					...
					if (ns.getServerSecurityLevel(target) > ns.getServerMinSecurityLevel(target) + 3) {
						...
						if (available_threads >= 1) {
							ns.exec(`${location}bin.wk.js`, server, available_threads, target)
						}
					} else {
						...
						// hack the target
						if (available_threads >= 1) {
							ns.exec(`${location}bin.hk.js`, server, available_threads, target)
						}
					}
